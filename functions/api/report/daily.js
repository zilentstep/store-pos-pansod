...
export async function onRequest(context) {
  try {
    ...
    const orderItemsRes = await env.DB.prepare(
      `SELECT oi.* FROM order_items oi JOIN orders o ON o.id = oi.order_id WHERE date(o.created_at) = ?`
    ).bind(date).all();
    ...
    const grabItemsRes = await env.DB.prepare(
      `SELECT goi.* FROM grab_order_items goi JOIN grab_orders go ON go.id = goi.grab_order_id WHERE date(go.created_at) = ?`
    ).bind(date).all();
